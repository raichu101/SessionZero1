...
async function hashPassword(pw, salt) {
  return sha256(salt + "::" + pw + "::sessionzero");
}
...
export default function App() {
  ...
  const [settings, setSettings] = useState({
    shopName: "SESSION ZERO", paypalEmail: "you@paypal.com", stripeKey: "",
    adminPassHash: null, adminSalt: null,
    fromName: "Session Zero", fromAddress: "", fromCity: "", fromState: "", fromZip: "",
  });
  ...
  useEffect(() => {
    (async () => {
      // Load products and orders from server, local data for session/cart
      let serverProducts = null;
      let serverOrders = null;
      let serverUsers = null;
      let serverSettings = null;
      try {
        const data = await api("data", { action: "load" });
      ...
      setProducts(serverProducts || LS.get("s-products") || DEMO_PRODUCTS);
      setOrders(serverOrders || LS.get("s-orders") || []);
      const loadedUsers = serverUsers || LS.get("s-users") || [];
      setUsers(loadedUsers);

      const s = serverSettings || LS.get("s-settings");
      if (s) setSettings(prev => ({ ...prev, ...s }));

      setCart(LS.get("s-cart") || []);

      const sess = LS.get("s-session");
      ...
      if (!s?.adminPassHash) {
        ...
        setSettings(ns); LS.set("s-settings", ns);
      }
      setReady(true);
    })();
  }, []);
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      if (session && !isSessionValid(session)) { setSession(null); setUser(null); LS.del("s-session"); notify("Session expired"); }
  ...
  /* ═══════════ PERSISTENCE ═══════════ */
  const saveToServer = useCallback(async (products, orders, users, settings) => {
    try { await api("data", { action: "save", products, orders, users, settings }); } catch {}
  }, []);

  const saveProducts = (val) => { setProducts(val); LS.set("s-products", val); saveToServer(val, null, null, null); };
  const saveOrders = (val) => { setOrders(val); LS.set("s-orders", val); saveToServer(null, val, null, null); };
  const saveUsers = (val) => { setUsers(val); LS.set("s-users", val); saveToServer(null, null, val, null); };
  const saveSettings = (s) => { setSettings(s); LS.set("s-settings", s); saveToServer(null, null, null, s); };
  const saveCart = (val) => { setCart(val); LS.set("s-cart", val); };
  ...
  const handleAuth = async () => {
    ...
    if (mode === "register") {
      ...
      setSession(sess); LS.set("s-session", sess);
  ...
  const logout = () => { setSession(null); setUser(null); LS.del("s-session"); goPage("shop"); notify("Signed out"); };
  ...
  const placeOrder = async () => {
    ...
    if (checkout.payMethod === "card") {
      const n = cardInfo.number.replace(/\s/g, "");
      if (n.length < 13 || !cardInfo.exp || !cardInfo.cvc || !cardInfo.name) { notify("Fill card fields"); return; }
      ...
      try {
        const result = await api("payment", {
          ...
          currency: "usd",
          description: `Session Zero order - ${cart.map(i => `${i.qty}x ${i.name}`).join(", ")}`,
          customer_email: sh.email,
          card: { number: n, exp: cardInfo.exp, cvc: cardInfo.cvc, name: cardInfo.name },
        });
        ...
        const order = {
          ...
          customer: sanitizeObj({ ...sh, userId: user?.id || null }),
          payment: { method: "card", last4: n.slice(-4), chargeId: result.chargeId || null },
        ...
        saveOrders([order, ...orders]); saveCart([]);
        setCardInfo({ number: "", exp: "", cvc: "", name: "" });
      ...
      // PayPal flow - record order after PayPal payment
      const order = {
        id: uid(), items: [...cart], total: cartTotal,
        customer: sanitizeObj({ ...sh, userId: user?.id || null }),
  ...
  return (
    <div className="app-root">
      ...
      <main className="main">
        ...
        {page === "checkout" && <>
          ...
          {checkout.step === 2 && <div className="ck-section">
            ...
            </div><div className="secure-note">🔒 Payments processed securely via Stripe. Card details are never stored.</div></div>}
          ...
          {checkout.step === 3 && <div className="ck-section">
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 20 }}>
              ...
              {checkout.payMethod === "paypal"
                ...
                : <button className="btn-primary btn-block" style={{ flex: 1 }} disabled={processingPayment} onClick={placeOrder}>{processingPayment ? "Processing Payment…" : `Place Order — $${cartTotal.toFixed(2)}`}</button>}
            </div>
        ...
        {/* ═══════ ORDER CONFIRMED ═══════ */}
        {page === "order-confirmed" && <div className="confirmed"><div className="confirmed-icon">✓</div><h2 className="confirmed-h">Order Confirmed</h2><p className="confirmed-p">Order #{viewOrder?.id}</p><p className="confirmed-sub">A confirmation has been sent to your email. You can track your order in your account.</p><button className="btn-primary" style={{ marginTop: 20 }} onClick={() => goPage("shop")}>Keep Shopping</button></div>}
        ...
        {page === "admin" && adminAuth && <>
          ...
          <div className="tab-bar">{["products", "orders", "customers", "shipping", "settings"].map(t => (
          ...
          {adminTab === "shipping" && <div className="panel">
            <h3>Shipping Labels</h3>
            <p className="td-dim" style={{ marginTop: 8, marginBottom: 16 }}>Select an order from the Orders tab to generate a shipping label, or generate labels for pending orders below.</p>
            {orders.filter(o => !o.shipped).length === 0 ? <div className="empty" style={{ padding: 30 }}><div className="empty-i">✓</div><div>All orders shipped!</div></div> :
            <div className="ship-queue">{orders.filter(o => !o.shipped).map(o => (
              <div key={o.id} className="ship-item">
                <div><span className="order-id">#{o.id}</span> — <strong>{o.customer.name}</strong></div>
                <div className="td-dim">{o.customer.city}, {o.customer.state} {o.customer.zip}</div>
                <div className="ship-item-right">
                  <span className="order-total">${o.total.toFixed(2)}</span>
                  <button className="btn-ship" onClick={() => setShipOrder(o)}>Generate Label</button>
                </div>
              </div>
            ))}</div>}
          </div>}
          ...
          {adminTab === "settings" && <div className="panel">
            ...
            <div className="fg" style={{ marginTop: 16 }}>
              {[["Store Name", "shopName"], ["PayPal Email", "paypalEmail"], ["Stripe Secret Key", "stripeKey"]].map(([l, k]) => (
                <div key={k} className="fg-full"><label className="fl">{l}</label><input className="fi" type={k === "stripeKey" ? "password" : "text"} value={settings[k]} onChange={e => setSettings(s => ({ ...s, [k]: e.target.value }))} /></div>))}
            </div>
            ...
            <div className="divider" /><h4 className="ck-h" style={{ fontSize: 16, marginBottom: 12 }}>Return Address (for shipping labels)</h4>
            <div className="fg">
            ...
            <div className="setup-guide">
              <h4>💳 Stripe Setup Instructions</h4>
              <p>1. Create a free account at <strong>stripe.com</strong></p>
              <p>2. Go to Developers → API Keys</p>
              <p>3. Copy your <strong>Secret Key</strong> (starts with sk_)</p>
              <p>4. Paste it in the "Stripe Secret Key" field above</p>
              <p>5. Set the key as <strong>STRIPE_SECRET_KEY</strong> environment variable in Netlify dashboard</p>
              <p style={{ marginTop: 8, color: "var(--accent)" }}>Stripe charges ~2.9% + $0.30 per transaction — no monthly or platform fees.</p>
              <p>Cards accepted: Visa, Mastercard, Amex, Discover, and debit cards.</p>
            </div>
            <div className="setup-guide" style={{ marginTop: 12 }}>
              <h4>🅿️ PayPal Setup</h4>
              <p>1. Enter your PayPal email above</p>
              <p>2. Customers will be redirected to PayPal to complete payment</p>
              <p>3. After paying, they click "Record Order" to finalize</p>
            </div>
          </div>}
        </>}
      </main>
      ...
      <footer className="footer"><span>© {new Date().getFullYear()} {settings.shopName}</span><span>🔒 Secure payments via Stripe & PayPal</span></footer>
    </div>
  );
}
...
function ShipModal({ order, settings, onShip, onClose }) {
  const tracking = `SZ${order.id.toUpperCase()}${Date.now().toString(36).toUpperCase().slice(0, 6)}`;
  return <div className="overlay" onClick={onClose}><div className="modal modal-lg" onClick={e => e.stopPropagation()}>
    ...
    <div className="label-paper">
      <div className="label-row"><div><div className="label-hd">FROM</div><p className="label-bold">{settings.fromName || settings.shopName}</p><p>{settings.fromAddress || "Set in Settings"}</p><p>{settings.fromCity}{settings.fromState ? ", " + settings.fromState : ""} {settings.fromZip}</p></div>
        <div style={{ textAlign: "right" }}><div className="label-hd">DATE</div><p className="label-bold">{new Date().toLocaleDateString()}</p><div className="label-hd" style={{ marginTop: 8 }}>ORDER</div><p className="label-bold">#{order.id}</p></div></div>
      ...
      <div className="label-contents"><div className="label-hd">CONTENTS</div>{order.items.map(i => <p key={i.id}>{i.qty}× {i.name} — ${(i.price * i.qty).toFixed(2)}</p>)}<p className="label-bold" style={{ marginTop: 6 }}>Declared Value: ${order.total.toFixed(2)}</p></div>
      <div className="label-barcode"><div className="barcode-lines">{"| || ||| || | ||| || | || ||| | || ||| || | ||| || |"}</div><div className="barcode-num">{tracking}</div></div>
    </div>
    <div style={{ display: "flex", gap: 10, justifyContent: "center", marginTop: 16 }}>
      <button className="btn-ghost" onClick={() => window.print()}>🖨️ Print Label</button>
...
.confirmed-p{color:var(--dim);margin-top:8px;font-family:var(--mono)}
.confirmed-sub{color:var(--faint);margin-top:8px;font-size:14px}
...
.ship-queue{display:flex;flex-direction:column;gap:10px}
.ship-item{display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;background:var(--s2);border:1px solid var(--s3);border-radius:var(--rs);padding:14px 16px}
.ship-item-right{display:flex;align-items:center;gap:12px}
